...
x_joystick = ADC(Pin(18))  # Links/Rechts Lenkung
x_joystick.atten(ADC.ATTN_11DB)

y_joystick = ADC(Pin(16))  # Vorwärts/Rückwärts Geschwindigkeit
...
def scale_forward_reverse_value(adc1_value, adc2_raw):
    ...
    range_min = -45 - (adc1_value * 70 / 100)
    range_max = 65 + (adc1_value * 70 / 100)
...
try:
    while True:
        ...
        display.show()
        time.sleep_ms(20)  # Sende alle 10ms
except:
    display.fill(0)
    display.show()
    pass